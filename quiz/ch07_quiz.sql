...
-- 정답: 4(UNION ALL 이 아닌 UNION 과 같음)
...
-- 정답:
SELECT m.name, p.date_of_birth, p.address
FROM members m JOIN member_profiles p ON m.id = p.member_id;
...
-- 정답:
SELECT m.name, p.date_of_birth, p.address
FROM members m LEFT JOIN member_profiles p ON m.id = p.member_id;
...
-- 정답:
SELECT m.name, k.title
FROM members m JOIN borrow_records b ON m.id = b.member_id
...
-- 정답:
SELECT b.title, r.borrow_date
FROM books b LEFT JOIN borrow_records r ON b.id = borrow_records.book_id; 
...
-- 정답:
-- 관계 설정 없어도 JOIN 할 수 있다는 것을 보여줌, 일반적으로는 관계 있는 테이블끼리 JOIN 수행
SELECT l.name, l.role
FROM library_staff l JOIN members m on l.name = m.name;
