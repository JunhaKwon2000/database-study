...
-- 정답: 3(AUTO_INCREMENT 는 무결성이라기 보다는 편의성을 위한 기능임)
...
-- 정답: 2
-- 한 명의 회원은 여러 대출 기록을 가질 수 있지만, 한 대출 기록은 오로지 한 명의 회원과 연관됨 = 1:N 관계
...
-- ② 회원과 대출 기록 간의 관계를 일대다로 정의한다. (논리)
-- ③ 회원 테이블과 대출 기록 테이블 간의 ER 다이어그램을 작성한다. (논리)
-- ④ 대출 기록 테이블에서 borrow_date 칼럼을 생성한다. (논리)
...
-- 각 인스턴스는 고유하므로 1정규형 만족
-- 기본키 후보는 두 컬럼의 복합키이며
-- 이 때 member_name 은 member_id 에만 종속되고,
-- book_title 과 category 는 book_id 에만 종속됨
-- 따라서 이 경우 부분 함수 종속이 존재하므로, 제 2정규형 위반임
