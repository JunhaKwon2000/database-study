...
-- 정답: 3
-- AND가 OR보다 우선순위가 높으므로  price > 50000 AND stock <= 20 이 먼저, 그 결과와 price < 20000 OR 를 비교
...
-- 정답:
SELECT name AS 제품명, price AS 가격 FROM products
WHERE price >= 300000;
...
-- 정답:
SELECT name AS 제품명, stock AS 재고 FROM products
...
-- 정답:
SELECT name AS 제품명, 1.1 * price AS '최종 가격' FROM products;
...
-- 정답:
SELECT name AS 제품명, category AS 카테고리 FROM products
...
-- 정답:
SELECT name AS 제품명, price * 0.8 AS 가격 FROM products
WHERE stock <= 10;
