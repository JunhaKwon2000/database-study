...
-- UNSIGNED 제약 조건: 음수 값을 허용하지 않고 양수 범위를 2배로
-- CREATE TABLE 테이블명 (컬럼명 데이터타입 UNSINGED);
...
-- ALTER TABLE user_profiles RENAME COLUMN intro TO self_introduction;
-- ALTER TABLE user_profiles RENAME COLUMN profile_pics TO profile_picture;
...
-- FLOAT과 DOUBLE 타입은 0.1을 정확히 저장하지 않고 근사값으로 저장(왜? 0.1은 2진수로 무한소수, 뒤에 부분을 짤라버리기 때문에 precise 하지 않은 값이 저장됨)
-- FLOAT: 약 7자리까지 정확함
-- DOUBLE: 약 15 ~ 17자리까지 정확함 (메모리는 더 먹긴 함 FLOAT 보다)

-- 고정 소수점: 특정 범위 안에서 정확한 연산을 수행(10진수 기반이라서)
-- DECIMAL 타입은 0.1을 정확하게 저장함
...
CREATE TABLE student_records (
	id INT PRIMARY KEY, -- 아이디(표준 정수, 기본키)
	grade TINYINT UNSIGNED, -- 학년(매우 작은 정수: 1 ~ 12, 범위는 0 ~ 255, 음수 없음)
	average_score FLOAT, -- 평균 점수(부동 소수점 방식의 실수)
	tuition_fee DECIMAL(10, 2) -- 수업료(고정 소수점 방식의 실수), 돈 관련된 값들은 정확해야함 -> DECIMAL 사용
);
...
INSERT INTO student_records 
VALUES (3, -2, 66.5, 20.00); -- 에러, grade -> out of range
INSERT INTO student_records 
VALUES (3, 256, 66.5, 20.00); -- 에러, grade -> out of range

INSERT INTO student_records 
VALUES (4, 200, 66.5, 20.12345); 
...
CREATE TABLE addresses (
	postal_code CHAR(5), -- 우편번호(고정길이 문자: 5자), 주로 고정된 길이의 코드값(예 : 국가코드)
    -- 문자를 3개만 넣는 경우, 자동으로 공백(space) 문자를 채움(예: 'abc  ')
    street_address VARCHAR(100) -- 거리 주소(가변길이 문자: 최대 100자)
...
CREATE TABLE files(
    ...
    large_date LONGBLOB -- 대용량 파일(최대 4GB)
);

-- (참고)
-- 자바 웹 개발을 포함한 대부분의 애플리케이션에서는 이미지나 동영상 같은 대용량 파일을
-- 데이터베이스에 직접 저장하지 않고, 클라우드 스토리지나 파일 서버 들에 저장한 뒤,
-- 그 경로나 URL만 데이터베이스에 저장하는 방식이 사실상 표준

-- 왜 DB에 직접 저장하지 않을까?
-- DB 부하가 큼, 성능이 느림, 백업 + 복구 어려움 등등
...
INSERT INTO user_profiles VALUES (3, 'hongsoon@example.com', '098-7654-3210', 'asd', NULL, '킹');
...
-- 1. 문자열 필터링하기
-- LIKE: 특정 패턴 찾기 -> SELECT * FROM table WHERE column LIKE 'pattern';
-- LIKE: wildcard(%: 0개 이상의 임의의 문자, _: 임의의 문자 1개)
-- LIKE: LIKE '케첩%' -> '케첩'으로 시작, LIKE '%케첩' -> '케첩'으로 끝남, LIKE '%케첩%' -> '케첩'을 포함
-- LIKE: LIKE '케첩_' -> '케첩'으로 시작하는 3글자, LIKE '_케첩' -> '케첩'으로 끝나는 3글자
SELECT * FROM orders WHERE name LIKE '%케첩%';
SELECT * FROM orders WHERE name LIKE '국내산%';

-- 2. 날짜 필터링하기
-- YEAR(날짜): 입력 날짜의 '연도' 추출 -> YEAR('2024-12-25') = 2024
-- MONTH(날짜): 입력 날짜의 '월' 추출 -> MONTH('2024-12-25') = 12
-- DAY(날짜): 입력 날짜의 '일' 추출 -> MONTH('2024-12-25') = 25
-- EXTRACT(필드 FROM 날짜): 입력된 날짜에서 특정 필드 추출 -> EXTRACT(YEAR FROM '2024-12-25') = 2024
SELECT SUM(quantity) FROM orders WHERE MONTH(created_at) = 11;

-- 3. 시간 필터링하기
-- HOUR(시간): 입력 시간의 '시' 추출 -> HOUR('10:15:24') = 10
-- MINUTE(시간): 입력 시간의 '분' 추출 -> MINUTE('10:15:24') = 15
-- SECOND(시간): 입력 시간의 '초' 추출 -> DAY('2024-12-25 10:15:24') = 24
-- TIME_TO_SEC(시간): 입력 시간을 '초'로 변환 -> TIME_TO_SEC('08:30:45') = 30645 
SELECT SUM(price * quantity) FROM orders WHERE HOUR(created_at) < 12;

-- 4. 특정 범위 필터링하기
-- BETWEEN: 두 값 사이에 속하는지 확인할 때 사용하는 연산자(두 값 포함) -> SELECT * FROM table WHERE column BETWEEN start AND end;
-- BETWEEN: 
SELECT * FROM orders WHERE price BETWEEN 10000 AND 200000;
SELECT COUNT(*) FROM orders WHERE created_at BETWEEN '2024-11-15' AND '2024-12-15';
SELECT * FROM orders WHERE name BETWEEN 'ㄱ' AND '깋';
SELECT * FROM orders WHERE name >= 'ㄱ' AND name < 'ㄴ';
...
-- 정답: (1) LIKE, (2) YEAR(), (3) EXTRACT(), (4) BETWEEN, (5) TIME_TO_SEC()
