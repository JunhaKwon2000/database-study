...
SELECT *, MAX(price), MIN(price) 
FROM burgers;
-- 집계 함수와 일반 컬럼들은 동시에 조회할 수 없음, 집계는 그룹 단위 연산이고 일반 컬럼은 행 단위 열람이기 때문
...
-- COUNT(*): 전체 레코드의 수를 반환(NULL 포함)
-- COUNT(컬럼): 해당 컬럼에서 NOT NULL 인 레코드의 수를 반환
...
CREATE TABLE transactions (
    ...
    amount DECIMAL(12, 2), -- 거래 금액(12자릿수: 정수 10자리 ,소수점 이하는 2자리까지 허용)
    msg VARCHAR(15),       -- 거래처
...
-- 거래 금액의 총합 구하기
SELECT SUM(amount), SUM(ABS(amount)) 
FROM transactions;
...
SELECT AVG(amount)
FROM transactions
WHERE SIGN(amount) = 1 AND msg IN ('Google', 'Amazon');
...
-- 정답: (1) DECIMAL, (2) DATETIME, (3) AVG(), (4) IN, (5) DISTINCT
