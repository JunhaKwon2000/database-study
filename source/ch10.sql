...
-- '무엇'을 만들 것인가에 집중
-- 엔티티 + 엔티티간의 관계 가 어떻게 되는지를 정의하는 단계

-- 예: 쇼핑몰 사이트
-- 엔티티: 사용자, 주문, 결제, 상품 -> 나중에 테이블이 될 수 있는 친구들
...
-- 예: 
-- 엔티티의 속성 및 식별자 정의(테이블의 구체적인 구성 요소들을 정의하는 단계)
-- 관계의 유형을 정의(1:1, 1:N, N:N 관계인지 등등 관계의 세부적인 사항을 정의)
-- 데이터의 중복 제거
-- 정규화(데이터가 잘 만들어졌는지 검증하는 작업)
...
-- 기본키 및 외래키 지정
-- 자료형, 제약 조건 등을 설정
-- => 실제 물리적으로 위 단계들에서 DB 에 설계한 테이블을 구체화해서 집어 넣는 작업 == 특정 DBMS 에 최적화한다고 함
...
CREATE TABLE users (
	id INTEGER, -- 기본키
	email VARCHAR(100) NOT NULL UNIQUE, -- 대체키(NOT NULL UNIOUE): 데이터베이스에서 후보키 중 기본키로 선택되지 않은 키
	name VARCHAR (50),
	PRIMARY KEY (id)
);
CREATE TABLE orders (
...
-- 정답: (1) 엔티티, (2) 인스턴스, (3) 속성, (4) 카디널리티, (5) 기본 식별자
...
-- users 테이블 생성
CREATE TABLE users (
	id INT AUTO_INCREMENT, -- 아이디(1씩 증가)
...
CREATE TABLE orders (
    ...
    user_id INT NOT NULL, -- 유저 아이디(users 테이블의 id 를 참조, 주문은 반드시 유저가 있어야 하므로(필수적 1대1 관계) NOT MULL 필요)
    PRIMARY KEY (id), -- 기본키(id)
    FOREIGN KEY (user_id) REFERENCES users (id) -- 외래키(users 테이블의 id 를 참조하는 user_id)
);
DESC orders;
...
CREAtE TABLE payments(
	id INT AUTO_INCREMENT, -- 아이디(1씩 증가)
    amount INT NOT NULL, -- 결제 금액(필수 입력)
    payment_type VARCHAR(50) NOT NULL, -- 결제 유형(필수 입력)
    order_id INT NOT NULL UNIQUE, -- 주문 아이디(핈수 입력, 고유키(필수적 1대1 관계) 지정)
    PRIMARY KEY (id), -- 기본키(id)
    FOREIGN KEY (order_id) REFERENCES orders (id) -- 외래키(orders 테이블의 id 를 참조하는 order_id)
);
DESC payments;
...
CREATE TABLE products(
	id INT AUTO_INCREMENT, -- 아이디(1씩 증가)
    name VARCHAR(50) NOT NULL UNIQUE, -- 상품명(필수 입력, 고유키 지정) = 후보키(NOT NULL UNIQUE) 이자 대체키(기본키가 아닌 후보키)
    price INT NOT NULL CHECK (price > 0), -- 가격(필수 입력, 양수만)
    product_type VARCHAR(50) DEFAULT 'NONE', -- 상품 유형(입력값이 없을 경우 NONE 이 기본값으로 들어감, NULL 명시적으로 넣으면 기본값 안들어갑!! NULL 도 하나의 값이자너)
    PRIMARY KEY (id), -- 기본키(id)
...
-- order_deatils 테이블 생성
CREATE TABLE order_details (
	id INT AUTO_INCREMENT, -- 아이디(1씩 증가)
	order_id INT NOT NULL, -- 주문 아이디(필수 입력(필수적 관걔))
    product_id INT NOT NULL, -- 상품 아이디(필수 입력(필수적 관걔))
    count INT NOT NULL CHECK(count > 0), -- 수량
    PRIMARY KEY (id), -- 기본키(id)
    FOREIGN KEY (order_id) REFERENCES orders (id), -- 외래키(orders 테이블의 id 를 참조하는 order_id)
    FOREIGN KEY (product_id) REFERENCES products (id), -- 외래키(products 테이블의 id 를 참조하는 product_id)
    UNIQUE (order_id, product_id) -- 일단 두 컬럼은 각각은 중복이 되는데, 두 컬럼의 조합은 고유해야된다는거임!COMMENT
    -- 즉, 하나의 주문에 같은 상품이 중복되지 않도록 제약을 걸어주는 거임!
    -- == 한 주문에서 하나의 상품이 여러개 담길 경우 count 로 관리하라는 뜻!
);
DESC order_details;
...
-- 정답: (1) 선택성, (2) NOT NULL, (3) CHECK, (4) DEFAULT, (5) 인덱스
...
-- 정답: (1) 정규화, (2) 원자화, (3) 완전 함수 종속, (4) 부분 함수 종속, (5) 이행적 함수 종속