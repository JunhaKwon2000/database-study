...
-- SELECT FROM table WHERE condition GROUP BY column HAVING condition_for_group;
-- 그룹화를 하면 SELECT 에 올 수 있는 컬럼에 제약이 생김 -> 그룹화를 진행한 컬럼 + 집계 함수(다른 컬럼) 만 가능
SELECT gender, AVG(height)
FROM students
GROUP BY gender; -- 성별을 기준으로 구룹화하겠다

-- 그룹화의 특징 3가지
-- 1) 집계 함수와 함께 사용해야 함
-- 그룹별 유의미한 분석을 얻기 위해서는 집계 함수 사용
-- 단순 GROUP BY 절만 사용하는 것은 데이터를 그룹으로 묶기만 함(의미X)
SELECT gender FROM students GROUP BY gender;

-- 2) 여러 컬럼으로 그룹화할 수 있다.
-- SELECT column1, column2, aggregate() 
-- FROM table WHERE condition 
-- GROUP BY column1, column2 HAVING condition_for_group;
-- 예: 특정 도시의 연도별 총매출 집계
-- sales 테이블 생성
CREATE TABLE sales (
	id INTEGER AUTO_INCREMENT,  	-- 아이디(자동으로 1씩 증가)
...
-- 특정 도시의 연도별 총매출 집계
SELECT city, YEAR(sale_date) AS year, SUM(amount) AS total_sales -- (3) 원하는 결과를 조회 
FROM sales -- (1) sales 테이블을 가져와서
GROUP BY city, YEAR(sale_date); -- (2) 도시와 연도로 그룹화한 이후
...
-- 잘못된 컬럼 사용 예시
SELECT id, gender, AVG(height) FROM students GROUP BY gender; -- id 컬럼 사용 못함
...
-- FROM table
-- WHERE normal_filter_condition -- 그룹화 하기 전에 개별 행(row)에 대해 필터링하는 행 단위 필터링
-- GROUP BY group_col
-- HAVING group_filter_condition -- 그룹핑된 결과에 대해 필터링하는 그룹 단위 필터링
...
SELECT ptype AS '결제 유형', AVG(amount) AS '평균 결제 금액'
FROM payments
-- WHERE AVG(amount) > 40000 -- WHERE 절은 GROUP BY나 집계 함수가 처리되기 전에 실행되기 떄문에 WHERE 절에는 집계함수가 못 옴
GROUP BY ptype
...
-- ORDER BY 절을 사용하여 수행
-- SELECT 로 조회된 데이터를 기준으로 정렬하는 작업(ORDER BY 는 SELECT 이후에 실행됨)

-- SELECT * 
-- FROM table 
-- WHERE cond ORDER BY column1, column2, ...[ASC | DESC];
-- ASC: 오름차순(default), DESC: 내림차순

-- 단일 컬럼 정렬: 결제 테이블에서 결제 금액이 높은 순서대로 조회하려면?
-- 결제 금액에 대한 정렬: 
SELECT ptype AS type, amount AS $ -- (2) 필요한 컬럼을 선택 및 별칭을 부여하고
FROM payments -- (1) 해당 테이블에서
ORDER BY amount DESC; -- (3) amount 기준으로 내림차순 정렬을 함

-- 다중 컬럼 정렬: 결제 테이블에서 결제 유형은 오름차순, 결제 금액은 내림차순으로 정렬하려면?
SELECT ptype AS type, amount AS $
FROM payments 
ORDER BY type, $ DESC; -- ptype의 오름차순 정렬 후, amount의 내림차순 정렬(순서 중요)

-- 3. 조회 개수 제한(LIMIT, OFFSET)
-- 조회 결고 중 상위 N개의 레코드만을 조회하는 명령
...
-- OFFSET 키워드 생략 가능, 생략시 LIMIT M, N; 이렇게 M과 N의 순서가 바뀜

-- 결제 금액 4 ~ 6 등까지 조회
SELECT * -- (2) 모든 컬럼을 선택한 후
FROM payments -- (1) 해당 테이블에서
ORDER BY amount DESC LIMIT 3 OFFSET 3; -- 내침차순으로 정렬한 후, 조회되는 행의 개수를 3개로 제한 근데 위의 3개의 행은 제외하고
...
-- 실제 개발에서는 이렇게 하드코딩하는 것이 아니라 OFFSET 계산 방법을 정의해 이를 넘김
...
-- 최신 목록부터 1페이지에 display 하게 order by 로 정렬

-- (참고) 성능 주의사항
-- OFFSET이 커질수록 성능이 떨어짐(데이터가 매우 많을 때)(건너뛸 데이터를 계속 읽기 때문)
...
-- 사용자 주문 총액 필터링 하기
-- 사용자별 주문 총액을 수하고, 그 총액이 30000원 이상인 주문자는?
SELECT u.nickname AS 주문자명, SUM(p.amount) AS '주문 총액'
FROM users u JOIN orders o ON u.id = o.user_id
JOIN payments p ON o.id = p.order_id
GROUP BY u.nickname HAVING sum(p.amount) >= 30000;
-- (1) FROM - JOIN: users - orders - payments table join
-- (2) GROUP BY: nickname 으로 그룹화
-- (3) HAVING: amount 의 sum 아 30000 이상인 친구들만 가져오도록 그룹 필터링
-- (4) SELECT: 원하는 컬럼 조회

-- 가장 많이 팔린 상품 TOP 3
-- 상품별 판매 수량을 집계했을 때, 가장 많이 팔리 상품 TOP 3
SELECT p.name AS 상품명, sum(od.count) AS '판매 수량' -- (6)
FROM products p -- (1)
JOIN order_details od ON p.id = od.product_id -- (2)
JOIN orders o ON od.order_id = o.id -- (3)
WHERE o.status = '배송 완료' -- (4)
GROUP BY p.name -- (5)
ORDER BY sum(od.count) DESC, p.name -- (7) 
LIMIT 3; -- (8)
-- Quiz: 코드의 실행 순서 달기
...
-- 정답: 쿼리문에 직접 작성함
-- 상품 가격 기준 이론상 매출액:실제 결제와 무관한 데이터(실제 결제는 구매자 직접 카드를 긁은 금액으로 해야함, 할인/부분결제/결제 누락 등을 전혀 반영하지 않았기 때문)
-- 실제 결제 금액 기반 매출액