...
2.3 데이터 삽입 및 조회하기
*/
-- 실습: 맥도날드 DB에 버거 테이블을 만들고, 다음 데이터를 삽임 및 조회
...
-- 정답: (1)CREATE TABLE, (2)INTEGER(INT), (3)VARCHAR, (4)INSERT INTO, (5)SELECT
...
*/
-- 데이터 수정하기: UPDATE
-- UDPATE 테이블명 SET 컬럼명 = 입력값 WHERE 조건; -- 조건을 만족하는 대상(row, 생략시 전체 row)에 대해서 어떤 컬럼에 어떤 값을 입력(수정)할지

-- 모든 레코드 수정하기
-- 모든 버거를 1000원에 판매하는 이벤트가 열렸다고 가정
-- 이를 위한 쿼리는?
...
-- 에러메시지 확인: safe update mode는 워크벤치의 보호 기능으로 KEY 없는 UPDATE/DELETE 차단함
...
-- 빅맨 버거만 가격을 500으로 수정
UPDATE burgers SET price = 500 WHERE name = '빅맨'; -- 특정 대상값 변경 시, 조건은 반드시 기본키(PK)를 사용
...
-- 데이터 삭제하기: DELETE
-- DELETE FROM 테이블명 WHERE 조건; -- 조건은 삭제 대상을 찾는 조건임

-- '슈비두밥 버거'가 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
DELETE FROM burgers WHERE name = '슈비두밥 버거'; -- 에러 발생: 안전모드로 인해 수정과 동일하게 조건은 반드시 기본키를 사용
