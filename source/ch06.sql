...
CREATE TABLE users(
    ...
    status VARCHAR(50) DEFAULT 'active', -- 계좌 상태(기본값은 'active') 
    balance INTEGER UNSIGNED, -- 계좌 잔액(음수 허용 X)
    age INT CHECK (age >= 18) -- 나이(18세 이상만 가입 허용)
);
...
-- 정답: (1) 관계, (2) 기본키(PK), (3) 불변성, (4) NULL, (5) 외래키(FK)

/*
6.2 다양한 관계 만들기
*/
-- 외래키와 기본키를 연결하여 일대일/일대다/다대다 관계를 만들어 보자구~

-- 1. one to one
-- A 테이블의 한 데이터가 B 테이블의 한 데이터와만 연결된 관계
-- 서로 긴밀한 연관성이 있거나, 하나의 테이블에서 분화된 경우의 관계
-- 외래키를 위치는 양쪽 테이블 중 어느 곳에 두어도 되지만, 이왕이면 사용빈도가 더 적은 쪽에 두는 것이 일반적임
...
CREATE TABLE capitals (
	id INT PRIMARY KEY, -- id (PK)
    name VARCHAR(255), -- capital name
    country_id INT UNIQUE, -- one to one 이므로 고유
    FOREIGN KEY (country_id) REFERENCES countries(id) -- FK -> countries 테이블의 id 를 참조하는 country_id
);
-- 다른 테이블의 컬럼을 참조하는 컬럼은 타입을 동일하게 해주기, 1대1 관계에서는 유니크 제약 추가하기
...
SELECT A.name AS '국가', B.name AS '수도'
FROM countries A JOIN capitals B ON A.id = B.country_id;

-- 2. 일대다 관계 만들기
-- A 테이블의 한 데이터가 B 테이블의 여러 데이터와 연결되는 관계
-- 하나의 데이터에 여러 데이터가 포함되거나 소유되는 경우
-- 일대다 관계에서는 Many 테이블에 외래키 지정
...
CREATE TABLE players (
    ...
    team_id INT, -- 1대다에서는 유니크 제약 X
    FOREIGN KEY (team_id) REFERENCES teams(id) -- 외래키 -> team_id 는 teams 테이블의 id 컬럼을 참조함
    -- 외래키를 통해 선수가 어느 팀에 소속되어 있는지 관계를 맺어줌
);
...
SELECT * FROM teams;
SELECT * FROM players;
SELECT t.name, p.name, p.id
FROM teams t JOIN players p ON t.id = p.team_id;


-- 3. 다대다 관계 만들기
-- A 와 B 테이블 서로가 다수의 데이터와 연결되는 관계
-- 중간 테이블을 경유하여 A 와 B 가 연결됨
...
CREATE TABLE patients (
	id INT PRIMARY KEY, -- id (PK)
    name VARCHAR(255) -- patients name
);

CREATE TABLE appointments (
	id INT, -- 아이디
    doctor_id INT, -- 의사 아이디
    patients_id INT, -- 환자 아이디
    date DATE, -- 진료 일자
    PRIMARY KEY (id), -- PK(기본키)
    FOREIGN KEY (doctor_id) REFERENCES doctors(id), -- 외래키 (의사 테이블의 id 를 참조하는 doctor_id)
    FOREIGN KEY (patients_id) REFERENCES patients(id) -- 외래키 (환자 테이블의 id 를 참조하는 patients_id)
);
...
SELECT d.name AS '의사명', a.date AS '진료일자', p.name AS '환자명'
...
CREATE TABLE likes (
	id INT AUTO_INCREMENT PRIMARY KEY, -- 아이디(자동으로 1씩 증가)
    user_id INT, -- 좋아요를 누른 사용자 아이디
    photo_id INT, -- 좋아요가 눌린 사진 아이디
    FOREIGN KEY (user_id) REFERENCES users(id), -- FK: users 테이블의 id 를 참조하는 user_id
    FOREIGN KEY (photo_id) REFERENCES photos(id) -- FK: users 테이블의 id 를 참조하는 user_id
);
...
-- 정답: (1) AUTO_INCREMENT, (2) NOT NULL, (3) 중간 테이블, (4) BOOLEAN, (5) UNIQUE