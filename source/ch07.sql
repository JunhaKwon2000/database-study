...
-- 조인 컬럼(조인을 수행할 때 기준이 되는 컬럼): 두 테이블이 동시에 가지고 있는 컬럼으로 조인하기 위해 사용하는 컬럼
...
-- inner join 하기 (join 만 적으면 inner join 으로 해석됨)
-- SELECT * 
-- FROM tableA join tableB on tableA.column = tableB.column

-- 1. 특정 사진에 달린 모든 댓글의 닉네임과 본문을 조회해보기: 1번 사진 댓글 + 닉네임 가져오기
SELECT u.nickname, c.body -- (4) SELECT
FROM comments c JOIN users u ON c.user_id = u.id -- (1) FROM -> (2) JOIN
WHERE photo_id = 1; -- (3) WHERE
-- JOIN users: 사용자 테이블을 붙일건데 -> ON comments.user_id = users.id: comments 테이블의 외래키와 users 테이블의 기본키가 같은 것들을 가져다줘 
-- FROM -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY

-- JOIN 의 특징
-- 1) 조인 컬럼이 필요(두 테이블을 연결하기 위한 공통 컬럼이 필요, 보통 외래키랑 기본키랑 조인함) -> JOIN users u ON c.user_id = u.id 에서 user_id 와 id 가 조인 컬럼
-- 2) 조인 컬럼은 서로 자료형이 일칭해야함(일치해야만 조인이 가능, 예를 들어 숫자 1이랑 문자열 '1'은 서로 조인이 불가능함)
-- 3) 조인 조건이 필요함(ON 절과 함께 사용, 두 테이블을 어떻게 연결할지를 조인 조건으로 명시)
-- 4) 연속적인 조인이 가능함
SELECT u.nickname, c.body, p.filename -- (4) 원하는 컬럼 조회
FROM comments c -- (1) 댓글 테이블에
JOIN users u ON c.user_id = u.id -- (2) 사용자 테이블 조인
JOIN photos p ON c.photo_id = p.id; -- (3) 사진 테이블 조인
-- 5) 중복 컬럼은 테이블명을 붙여 구분함(컬럼명이 같은 경우 어느 테이블의 것인지 명시필요, 구분 안해주면 에러 발생)
SELECT comments.id, users.id
-- SELECT id -- id 가 중복이자나요! 
FROM comments JOIN users ON comments.user_id = users.id
WHERE photo_id = 2;
-- 6) 테이블명에 별칭 사용 가능(간결한 쿼리 작성 및 가독성 향상에 도움)
SELECT u.nickname, c.body 
FROM comments c JOIN users u ON c.user_id = u.id 
WHERE photo_id = 1;
-- (참고) 테이블에 별칭을 붙일 때는 AS 를 거의 생략
-- 7) 다양한 조인 유형 사용 가능(다양한 결과 테이블 생성에 도움: INNER, LEFT OUTER, RIGHT OUTER, FULL OUTER, CROSS, NATURAL, SELF...)
...
-- 조인 조건을 만족하지 않는 경우, NULL 값으로 채움
-- LEFT 조인에서 기준만 바뀌고 동일하기에 거의 사용되지 않음 -> 그냥 테이블 위치만 바꾸면 됨

-- photos 테이블과 users 테이블 INNER 조인
SELECT *
FROM photos p RIGHT JOIN users u ON p.user_id = u.id;
...
-- 중복 튜플 제거하고 합치기
-- (쿼리A)
-- UNION -- 두 쿼리의 결과 테이블을 하나로 합치기(중복 데이터 = inner join 의 결과 레코드들 제거)
-- (쿼리B);
SELECT *
FROM photos p RIGHT JOIN users u ON p.user_id = u.id
UNION
...
-- 정답: (1) INNER JOIN, (2) LEFT JOIN, (3) RIGHT JOIN, (4) FULL JOIN, (5) UNION
...
SELECT c.body, p.filename
FROM comments c LEFT JOIN photos p ON c.photo_id = p.id;
